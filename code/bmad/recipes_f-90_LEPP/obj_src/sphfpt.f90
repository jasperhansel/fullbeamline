	...
	REAL(dp), PARAMETER :: DXX=1.0e-4_dp
	REAL(dp), DIMENSION(:), POINTER :: v1,v2
	REAL(dp), DIMENSION(NTOT), TARGET :: v
	LOGICAL(LGT) :: check
	v1=>v(1:N2)
	v2=>v(N2+1:NTOT)
	nn2=N2
	...
	do
		...
		v1(1)=n*(n+1)-m*(m+1)+c2/2.0_dp
		v2(2)=v1(1)
		v2(1)=gamma*(1.0_dp-(v2(2)-c2)*dx/(2*(m+1)))
		x1=-1.0_dp+dx
		x2=1.0_dp-dx
		xf=0.0
		call newt(v,check)
