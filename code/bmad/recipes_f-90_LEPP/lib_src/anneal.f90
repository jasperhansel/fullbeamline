	...
	REAL(dp) :: de,harvest,path,t,tfactr
	LOGICAL(LGT) :: ans
...
!BL
	...
	REAL(dp), DIMENSION(size(x1)) :: alen_v
	alen_v=sqrt((x2-x1)**2+(y2-y1)**2)
	END FUNCTION alen_v
!BL
	SUBROUTINE metrop(de,t,ans)
	IMPLICIT NONE
	REAL(dp), INTENT(IN) :: de,t
