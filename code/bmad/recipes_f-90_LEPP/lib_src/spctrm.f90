	...
	REAL(dp) :: den,facm,facp,sumw
	REAL(dp), DIMENSION(2*size(p)) :: w
	REAL(dp), DIMENSION(4*size(p)) :: w1
	REAL(dp), DIMENSION(size(p)) :: w2
	COMPLEX(dpc), DIMENSION(2*size(p)) :: cw1
	m=size(p)
	...
	do kk=1,k
		...
		cw1(1:mm)=cmplx(w1(1:m4:2),w1(2:m4:2),kind=dpc)
...
!BL
	...
	REAL(dp), INTENT(IN) :: facm,facp
	REAL(dp), DIMENSION(size(j)) :: window
	select case(nn_window)
		case(1)
			window(j)=(1.0_dp-abs(((j-1)-facm)*facp))
		case(2)
			window(j)=1.0
		case(3)
			window(j)=(1.0_dp-(((j-1)-facm)*facp)**2)
		case default
