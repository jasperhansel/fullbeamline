...
!BL
	...
	REAL(dp), DIMENSION(size(xx)) :: bessjgt_v
	npak=count(mask)
	if (npak == 0) RETURN
	...
	bessjgt_v=unpack(bj,mask,0.0_dp)
...
!BL
	...
	REAL(dp), DIMENSION(:), INTENT(IN) :: xx
	LOGICAL(LGT), DIMENSION(size(xx)), INTENT(IN) :: mask
	REAL(dp), DIMENSION(size(xx)) :: bessjle_v
	npak=count(mask)
	if (npak == 0) RETURN
	...
	bessjle_v=unpack(bessjle,mask,0.0_dp)
