	...
	REAL(dp) :: ave,ck,ckk,cterm,cwt,den,df,effm,expy,fac,fndim,hc2wt,&
		hs2wt,hypo,sterm,swt,var,xdif,xmax,xmin
	REAL(dp), DIMENSION(:), ALLOCATABLE :: wk1,wk2
	...
	fndim=ndim
	do j=1,n
		ck=1.0_dp+mod((x(j)-xmin)*fac,fndim)
		ckk=1.0_dp+mod(2.0_dp*(ck-1.0_dp),fndim)
		call spreadval(y(j)-ave,wk1,ck,MACC)
		call spreadval(1.0_dp,wk2,ckk,MACC)
	end do
	...
	do j=1,nout
		...
		hc2wt=0.5_dp*wk2(k)/hypo
		hs2wt=0.5_dp*wk2(k+1)/hypo
		cwt=sqrt(0.5_dp+hc2wt)
		swt=sign(sqrt(0.5_dp-hc2wt),hs2wt)
		den=0.5_dp*n+hc2wt*wk2(k)+hs2wt*wk2(k+1)
		cterm=(cwt*wk1(k)+swt*wk1(k+1))**2/den
		sterm=(cwt*wk1(k+1)-swt*wk1(k))**2/(n-den)
		px(j)=j*df
		py(j)=(cterm+sterm)/(2.0_dp*var)
		k=k+2
	end do
	deallocate(wk1,wk2)
...
!BL
	...
	n=size(yy)
	ix=x
	if (x == real(ix,dp)) then
		yy(ix)=yy(ix)+y
	else
		ilo=min(max(int(x-0.5_dp*m+1.0_dp),1),n-m+1)
		ihi=ilo+m-1
		nden=nfac(m)
