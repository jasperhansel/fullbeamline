...
!BL
	...
	IMPLICIT NONE
	REAL(dp) :: b1,b2,bb,chisq,del,f,f1,f2,sigb,sx,sxx,sxy,sy
...
!BL
	...
	REAL(dp), INTENT(IN) :: b
	REAL(dp) :: rofunc
	REAL(dp), PARAMETER :: EPS=epsilon(b)
	INTEGER(I4B) :: j
	REAL(dp), DIMENSION(size(x)) :: arr,d
	arr(:)=y(:)-b*x(:)
	if (mod(ndata,2) == 0) then
		j=ndata/2
		aa=0.5_dp*(select(j,arr)+select(j+1,arr))
	else
	...
	rofunc=sum(x(:)*sign(1.0_dp,d(:)), mask=(abs(d(:)) > EPS) )
	END FUNCTION rofunc
