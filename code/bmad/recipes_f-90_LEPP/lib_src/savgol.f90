	...
	REAL(dp) :: d,sm
	REAL(dp), DIMENSION(m+1) :: b
	REAL(dp), DIMENSION(m+1,m+1) :: a
	INTEGER(I4B) :: irng(nl+nrr+1)
	call assert(nl >= 0, nrr >= 0, ld <= m, nl+nrr >= m, 'savgol args')
	do ipj=0,2*m
		sm=sum(arth(1.0_dp,1.0_dp,nrr)**ipj)+&
			sum(arth(-1.0_dp,-1.0_dp,nl)**ipj)
		if (ipj == 0) sm=sm+1.0_dp
		mm=min(ipj,2*m-ipj)
		do imj=-mm,mm,2
	...
	np=nl+nrr+1
	savgol(mod(np-irng(:),np)+1)=poly(real(irng(:),dp),b(:))
	END FUNCTION savgol
