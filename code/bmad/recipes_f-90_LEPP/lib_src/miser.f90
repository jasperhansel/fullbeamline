	...
	REAL(dp), PARAMETER :: PFAC=0.1_dp,TINY=1.0e-30_dp,BIG=1.0e30_dp
	INTEGER(I4B), PARAMETER :: MNPT=15,MNBS=60
	REAL(dp), DIMENSION(:), ALLOCATABLE :: regn_temp
	INTEGER(I4B) :: j,jb,n,ndum,npre,nptl,nptr
	INTEGER(I4B), SAVE :: iran=0
	REAL(dp) :: avel,varl,fracl,fval,rgl,rgm,rgr,&
		s,sigl,siglb,sigr,sigrb,sm,sm2,sumb,sumr
	REAL(dp), DIMENSION(:), ALLOCATABLE :: fmaxl,fmaxr,fminl,fminr,pt,rmid
	...
	if (npts < MNBS) then
		...
		do j=1,ndim
			...
			s=sign(dith,real(iran-87500,dp))
			rmid(j)=(0.5_dp+s)*regn(j)+(0.5_dp-s)*regn(ndim+j)
		end do
		do n=1,npre
			call ranpt(pt,regn)
			fval=func(pt)
		...
		do j=1,ndim
			if (fmaxl(j) > fminl(j) .and. fmaxr(j) > fminr(j)) then
				sigl=max(TINY,(fmaxl(j)-fminl(j))**(2.0_dp/3.0_dp))
				sigr=max(TINY,(fmaxr(j)-fminr(j))**(2.0_dp/3.0_dp))
				sumr=sigl+sigr
				if (sumr <= sumb) then
					sumb=sumr
					jb=j
		...
		nptl=(MNPT+(npts-npre-2*MNPT)*fracl*siglb/ &
			(fracl*siglb+(1.0_dp-fracl)*sigrb))
		nptr=npts-npre-nptl
