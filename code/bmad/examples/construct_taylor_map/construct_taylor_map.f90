...
type (taylor_struct) xfer_map(6), spin_map(3,3)
...
do while (i < cesr_iargc())
  i=i+1; call cesr_getarg(i, arg)
  select case (arg)
  case ('-spin')
    include_spin = .true.
  case ('-from')
    i=i+1; i=i+1; call cesr_getarg(i, from_ele)
  case ('-out')
    i=i+1; i=i+1; call cesr_getarg(i, out_file)
  case default
    if (arg(1:1) == '-') then
...
if (include_spin) then
  call transfer_map_calc_with_spin (lat, xfer_map, spin_map, orb0(ixe), err, ixe, ixe, ele%ix_branch, .true.)
  call type_taylors (xfer_map, file_id = 1)
  call type_spin_taylors(spin_map, file_id = 1)
