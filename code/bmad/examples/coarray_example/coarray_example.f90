program coarray_example
  ...
  pz = 0.0
  CALL ptc_one_turn_map_at_ele(lat%ele(0), one_turn_map, .false., pz)
  ...
  do i=1,work_schedule(my_worker_num)
    ...
    do j=1,n_turns
      ...
      CALL track_taylor(orb(lat%n_ele_track)%vec, A_inverse, A_coords)
      CALL track_taylor(orb(lat%n_ele_track)%vec, A1_inverse, A1_coords)
