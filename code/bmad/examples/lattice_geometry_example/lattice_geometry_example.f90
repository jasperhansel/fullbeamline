...
call type_lat_floor(iu, 0.0_rp, 0.0_rp, in_ele_frame=.false.)
close(iu)
call type_header()
open(iu, file='element_frame.dat')
call type_lat_floor(iu, 0.0_rp, 0.0_rp, in_ele_frame=.true.)
close(iu)


open(iu, file='reference_boxes.dat')
call type_header()
call type_lat_floor(iu, 1*sc,  1*sc, in_ele_frame=.false.)
call type_lat_floor(iu, -1*sc, 1*sc, in_ele_frame=.false.)
call type_lat_floor(iu, -1*sc,-1*sc, in_ele_frame=.false.)
call type_lat_floor(iu,  1*sc,-1*sc, in_ele_frame=.false.)
close(iu)
...
call type_header()
call type_lat_floor(iu, 1*sc,  1*sc, in_ele_frame=.true.)
call type_lat_floor(iu, -1*sc, 1*sc, in_ele_frame=.true.)
call type_lat_floor(iu, -1*sc,-1*sc, in_ele_frame=.true.)
call type_lat_floor(iu,  1*sc,-1*sc, in_ele_frame=.true.)
close(iu)
...
subroutine type_lat_floor(iu, x, y, in_ele_frame)
real(rp) :: x, y
integer :: i, iu
logical, optional :: in_ele_frame
do i=1, lat%n_ele_track
  ele=>lat%ele(i)
  call type_floor(iu, ele, x,  y, in_ele_frame = logic_option(.false., in_ele_frame))
enddo
end subroutine
...
subroutine type_floor(iu, ele, x, y, in_ele_frame)
...
do i=0, n_steps-1
  ...
  f0%r = [x, y, s]
  f1= coords_local_curvilinear_to_floor (f0, ele, in_ele_frame = logic_option(.false., in_ele_frame), w_mat = w_mat)
