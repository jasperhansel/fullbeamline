...
C     MOD:
C          21-nov-2007, PT:
C             protect against bug when an expression is parsed
C             prior to the definition of any element parameters,
C             which results in IPARM2 pointing outside of the 
C             parameter table.  Back out old changes in RDLOOK
C             calls which were made as a workaround to a strange
C             bug in CVF 6.
C          15-DEC-2003, PT:
...
C-----------------------------------------------------------------------        
      integer*4 lname, ifun, ielem, lpara, iep1, iep2, iep, iparm   ! dcs
      real*8 value   ! dcs
C-----------------------------------------------------------------------        
      LOGICAL           FLAG     
      LOGICAL*4         MADE_NAMELESS_TEMPORARY
...
C---- ELEMENT PARAMETER?                                                        
        ELSE IF (KLINE(ICOL) .EQ. '[') THEN                                     
C          CALL RDLOOK(KNAME,ENAME_LENGTH,KELEM(1),1,IELEM1,IELEM)                             
          ...
          IEP1 = IEDAT(IELEM,1)                                                 
          IEP2 = IEDAT(IELEM,2)                                                 
C          CALL RDLOOK(KPARA,LPARA,KPARM(1),IEP1,IEP2,IEP)                          
...
C      IF (IVAL(1) .EQ. IPARM2 .AND. KPARM(IPARM2)(1:1) .EQ. '*') THEN
      MADE_NAMELESS_TEMPORARY = (IVAL(1).EQ.IPARM2)
      IF (MADE_NAMELESS_TEMPORARY) THEN
        MADE_NAMELESS_TEMPORARY = (MADE_NAMELESS_TEMPORARY .AND.
     &      KPARM(IPARM2)(1:1) .EQ. '*') 
      ENDIF
      IF (MADE_NAMELESS_TEMPORARY) THEN
