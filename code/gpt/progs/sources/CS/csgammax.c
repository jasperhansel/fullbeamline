/* CSgammax.c - Calculate Courant-Snyder gamma for x x' subspace in [rad/m] */
...
int CSgammax_func( double *result )
{
  int num, tmpnum ;
  ...
  if( emix==0.0 || avgBz==0.0 ) return( 1 ) ;

  *result = xpxp/(emix*avgBz) ;
