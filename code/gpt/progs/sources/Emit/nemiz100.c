/* nemiz100.c - Calculate 100% normalized rms emittance for t E subspace in [eV*s] */
...
int nemiz100_func( double *result )
{
  int i, num, tmpnum ;
  double *nmacro, *z,  *Bz,  *G,  *t,  *m, m0;
  double *tc, *Gc ;
  double GG, tt, tG ;
  double emiz, emiz100, emizi, alphaz, betaz, gammaz ;
  ...
  for(i=0 ; i<num ; i++)
  {
    emizi = gammaz*tc[i]*tc[i] + 2*alphaz*tc[i]*Gc[i] + betaz*Gc[i]*Gc[i] ;
    if( emizi>emiz100 ) emiz100 = emizi ;
  }
  *result = emiz100 * m0*gpt_c*gpt_c / -gpt_qe;
